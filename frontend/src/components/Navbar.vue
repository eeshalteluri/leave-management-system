<template>
  <header class="sticky top-0 z-50 border-b border-gray-800 bg-gray-950/80 backdrop-blur-xl">
      ...
      <div class="flex items-center justify-between h-16">
        ...
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 bg-brand-500 rounded-lg flex items-center justify-center">
            <svg class="w-4 h-4 text-gray-950" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
          </div>
          <span class="font-display font-bold text-white tracking-tight">LeaveDesk</span>
        </div>
        ...
        <div class="flex items-center gap-3">
          <div class="hidden sm:flex flex-col items-end">
            <span class="text-sm font-medium text-gray-200 leading-none">{{ auth.user?.name }}</span>
            <span class="text-xs text-gray-500 capitalize mt-0.5 font-mono">{{ auth.user?.role }}</span>
          </div>
