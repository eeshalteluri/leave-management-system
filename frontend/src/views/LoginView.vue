<template>
  <div class="min-h-screen bg-gray-950 flex items-center justify-center px-4 relative overflow-hidden">
    ...
    <div class="absolute inset-0 pointer-events-none">
      <div class="absolute top-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-brand-500/5 rounded-full blur-3xl" />
    </div>
    ...
    <div class="w-full max-w-md animate-fade-up relative z-10">
      ...
      <div class="flex flex-col items-center mb-10">
        <div class="w-14 h-14 bg-brand-500 rounded-2xl flex items-center justify-center mb-5 shadow-lg shadow-brand-500/20">
          <svg class="w-7 h-7 text-gray-950" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            <path stroke-linecap="round" stroke-linejoin="round"
              d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
        </div>
        <h1 class="text-3xl font-display font-bold text-white tracking-tight">LeaveDesk</h1>
      ...
      <div class="card border-gray-800">
        ...
        <form @submit.prevent="handleSubmit" novalidate class="space-y-5">
          ...
          <div>
            ...
            <input id="password" v-model="form.password" type="password" class="field"
              placeholder="••••••••" autocomplete="current-password" />
          </div>

          <button type="submit" class="btn-brand w-full mt-2" :disabled="loading">
        ...
        <p class="text-center text-sm text-gray-500">
          No account?
          <RouterLink to="/signup" class="text-brand-400 hover:text-brand-300 font-medium transition-colors">
            Create one
          </RouterLink>
        </p>
      </div>
